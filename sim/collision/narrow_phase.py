"""Narrow-phase dispatch for the Newton-compatible SAP collision front end.

Source note: the SAP modifications in this module are based on Newton's
narrow-phase collision code and adapted for SAP Warp contact buffers.
"""
...
from .contact_data import SAP_SHAPE_PAIR_HFIELD_BIT, SapContactData, sap_contact_passes_gap_check
from .core import (
...
from .global_reduction import (
    ...
    sap_mesh_triangle_contacts_to_reducer_kernel,
...
from .math import SAP_MAXVAL
from .primitive import (
    sap_collide_capsule_capsule,
    sap_collide_plane_box,
    sap_collide_plane_capsule,
    sap_collide_plane_cylinder,
    sap_collide_plane_ellipsoid,
    sap_collide_plane_sphere,
    sap_collide_sphere_box,
    sap_collide_sphere_capsule,
    sap_collide_sphere_cylinder,
    sap_collide_sphere_sphere,
)
from .sdf_contact import (
    sap_compute_mesh_mesh_block_offsets,
    sap_create_narrow_phase_process_mesh_mesh_contacts_kernel,
)
from .sdf_texture import SapTextureSDFData
from .support import SapSupportMapDataProvider, sap_extract_shape_data, sap_support_map_lean
from .types import SapGeoType


@wp.struct
class SapContactWriterData:
    contact_max: int
...
@wp.func
def sap_write_contact_simple(
    contact_data: SapContactData,
    writer_data: SapContactWriterData,
    output_index: int,
):
    """
    ...
    Args:
        contact_data: SapContactData struct containing contact information
        writer_data: SapContactWriterData struct containing output arrays
...
def sap_create_narrow_phase_primitive_kernel(writer_func: Any):
    """
    ...
    Args:
        writer_func: Contact writer function (e.g., sap_write_contact_simple)
    ...
    def sap_narrow_phase_primitive_kernel(
        ...
        for t in range(tid, num_work_items, total_num_threads):
            ...
            # =====================================================================
            is_hfield_a = type_a == SapGeoType.HFIELD
            is_hfield_b = type_b == SapGeoType.HFIELD

            if is_hfield_a or is_hfield_b:
                is_mesh_like_a = type_a == SapGeoType.MESH or is_hfield_a
                is_mesh_like_b = type_b == SapGeoType.MESH or is_hfield_b
                ...
                if is_mesh_like_a and is_mesh_like_b:
                    ...
                    if is_hfield_b:
                        encoded_a = shape_b | SAP_SHAPE_PAIR_HFIELD_BIT
                        encoded_b = shape_a
                    elif is_hfield_a:
                        encoded_a = shape_a | SAP_SHAPE_PAIR_HFIELD_BIT
                        encoded_b = shape_b
                    else:
                        encoded_a = shape_a
                        encoded_b = shape_b
            ...
            # =====================================================================
            is_mesh_a = type_a == SapGeoType.MESH
            is_mesh_b = type_b == SapGeoType.MESH
            is_plane_a = type_a == SapGeoType.PLANE
            ...
            # =====================================================================
            is_sphere_a = type_a == SapGeoType.SPHERE
            is_sphere_b = type_b == SapGeoType.SPHERE
            is_capsule_a = type_a == SapGeoType.CAPSULE
            is_capsule_b = type_b == SapGeoType.CAPSULE
            is_ellipsoid_b = type_b == SapGeoType.ELLIPSOID
            is_cylinder_b = type_b == SapGeoType.CYLINDER
            is_box_b = type_b == SapGeoType.BOX
            ...
            # Distances default to SAP_MAXVAL so unused slots are automatically
            # excluded by the unified num_contacts count after the if/elif chain.
            contact_dist_0 = float(SAP_MAXVAL)
            contact_dist_1 = float(SAP_MAXVAL)
            contact_dist_2 = float(SAP_MAXVAL)
            contact_dist_3 = float(SAP_MAXVAL)
            ...
            if is_plane_a and is_sphere_b:
                ...
                contact_dist_0, contact_pos_0 = sap_collide_plane_sphere(plane_normal, pos_a, pos_b, sphere_radius)
            ...
            # -----------------------------------------------------------------
                ...
                contact_dist_0, contact_pos_0, contact_normal = sap_collide_plane_ellipsoid(
            ...
            # -----------------------------------------------------------------
                ...
                dists4_box, positions4_box, contact_normal = sap_collide_plane_box(
            ...
            # -----------------------------------------------------------------
                ...
                radius_a = scale_a[0]
                radius_b = scale_b[0]
                contact_dist_0, contact_pos_0, contact_normal = sap_collide_sphere_sphere(pos_a, radius_a, pos_b, radius_b)
            ...
            # -----------------------------------------------------------------
                ...
                dists, positions, _frame = sap_collide_plane_capsule(
            ...
            # -----------------------------------------------------------------
                ...
                dists4, positions4, contact_normal = sap_collide_plane_cylinder(
            ...
            # -----------------------------------------------------------------
                ...
                capsule_radius = scale_b[0]
                capsule_half_length = scale_b[1]
                contact_dist_0, contact_pos_0, contact_normal = sap_collide_sphere_capsule(
            ...
            # -----------------------------------------------------------------
                ...
                dists, positions, contact_normal = sap_collide_capsule_capsule(
            ...
            # -----------------------------------------------------------------
                ...
                cylinder_radius = scale_b[0]
                cylinder_half_height = scale_b[1]
                contact_dist_0, contact_pos_0, contact_normal = sap_collide_sphere_cylinder(
            ...
            # -----------------------------------------------------------------
                ...
                box_rot = wp.quat_to_matrix(quat_b)
                box_size = scale_b
                contact_dist_0, contact_pos_0, contact_normal = sap_collide_sphere_box(
            ...
            # =====================================================================
            num_contacts = (
                int(contact_dist_0 < SAP_MAXVAL)
                + int(contact_dist_1 < SAP_MAXVAL)
                + int(contact_dist_2 < SAP_MAXVAL)
                + int(contact_dist_3 < SAP_MAXVAL)
            )
            if num_contacts > 0:
    ...
    return sap_narrow_phase_primitive_kernel


def sap_create_narrow_phase_kernel_gjk_mpr(
    ...
    @wp.kernel(enable_backward=False, module="unique")
    def sap_narrow_phase_kernel_gjk_mpr(
        candidate_pair: wp.array(dtype=wp.vec2i),
        candidate_pair_count: wp.array(dtype=int),
        shape_types: wp.array(dtype=int),
        shape_data: wp.array(dtype=wp.vec4),
        ...
        (no meshes, no simple primitives).
        """
        tid = wp.tid()
        ...
        for t in range(tid, num_work_items, total_num_threads):
            ...
            # Extract shape data
            pos_a, quat_a, shape_data_a, scale_a, margin_offset_a = sap_extract_shape_data(
                shape_a, shape_transform, shape_types, shape_data, shape_source
            )
            pos_b, quat_b, shape_data_b, scale_b, margin_offset_b = sap_extract_shape_data(
                shape_b, shape_transform, shape_types, shape_data, shape_source
            )

            # Check for infinite planes
            is_infinite_plane_a = (type_a == SapGeoType.PLANE) and (scale_a[0] == 0.0 and scale_a[1] == 0.0)
            is_infinite_plane_b = (type_b == SapGeoType.PLANE) and (scale_b[0] == 0.0 and scale_b[1] == 0.0)
            ...
            if has_infinite_plane:
                ...
                if wp.static(not external_aabb):
                    ...
                    if is_infinite_plane_a:
                        ...
                        aabb_a_lower, aabb_a_upper = sap_compute_tight_aabb_from_support(
                    ...
                    if is_infinite_plane_b:
                        ...
                        data_provider = SapSupportMapDataProvider()
                        aabb_b_lower, aabb_b_upper = sap_compute_tight_aabb_from_support(
                ...
                bsphere_center_a, bsphere_radius_a = sap_compute_bounding_sphere_from_aabb(aabb_a_lower, aabb_a_upper)
                bsphere_center_b, bsphere_radius_b = sap_compute_bounding_sphere_from_aabb(aabb_b_lower, aabb_b_upper)

                if not sap_check_infinite_plane_bsphere_overlap(
                    shape_data_a,
                    shape_data_b,
                    pos_a,
                    pos_b,
            ...
            wp.static(
                sap_create_find_contacts(writer_func, support_func=support_func, post_process_contact=post_process_contact)
            )(
                pos_a,
                pos_b,
                quat_a,
    ...
    return sap_narrow_phase_kernel_gjk_mpr


@wp.kernel(enable_backward=False)
def sap_narrow_phase_find_mesh_triangle_overlaps_kernel(
    shape_types: wp.array(dtype=int),
    shape_transform: wp.array(dtype=wp.transform),
    shape_source: wp.array(dtype=wp.uint64),
    ...
    heightfield_data: wp.array(dtype=SapHeightfieldData),
    shape_pairs_mesh: wp.array(dtype=wp.vec2i),
    shape_pairs_mesh_count: wp.array(dtype=int),
    total_num_threads: int,
    # outputs
    ...
    for i in range(tid, num_mesh_pairs, total_num_threads):
        ...
        if type_a == SapGeoType.MESH and type_b != SapGeoType.MESH:
            mesh_shape = shape_a
            non_mesh_shape = shape_b
        elif type_b == SapGeoType.MESH and type_a != SapGeoType.MESH:
            mesh_shape = shape_b
        ...
        # Call sap_mesh_vs_convex_midphase with the shape_data and pair gap sum.
        sap_mesh_vs_convex_midphase(
            j,
            mesh_shape,
            non_mesh_shape,
...
def sap_create_narrow_phase_process_mesh_triangle_contacts_kernel(writer_func: Any):
    @wp.kernel(enable_backward=False, module="unique")
    def sap_narrow_phase_process_mesh_triangle_contacts_kernel(
        shape_types: wp.array(dtype=int),
        shape_data: wp.array(dtype=wp.vec4),
        shape_transform: wp.array(dtype=wp.transform),
        shape_source: wp.array(dtype=wp.uint64),
        ...
        for i in range(tid, num_triangle_pairs, total_num_threads):
            ...
            if type_a == SapGeoType.HFIELD:
                ...
                shape_data_a, v0_world = sap_get_triangle_shape_from_heightfield(
                ...
                X_ws_a = shape_transform[shape_a]
                shape_data_a, v0_world = sap_get_triangle_shape_from_mesh(mesh_id_a, mesh_scale_a, X_ws_a, tri_idx)

            # Extract shape B data
            pos_b, quat_b, shape_data_b, _scale_b, margin_offset_b = sap_extract_shape_data(
                shape_b,
                shape_transform,
                shape_types,
                shape_data,
            ...
            wp.static(sap_create_compute_gjk_mpr_contacts(writer_func))(
                shape_data_a,
                shape_data_b,
                quat_a,
                quat_b,
    ...
    return sap_narrow_phase_process_mesh_triangle_contacts_kernel


@wp.kernel(enable_backward=False)
def sap_compute_mesh_plane_block_offsets(
    shape_pairs_mesh_plane: wp.array(dtype=wp.vec2i),
    shape_pairs_mesh_plane_count: wp.array(dtype=int),
    shape_source: wp.array(dtype=wp.uint64),
    target_blocks: int,
...
def sap_create_narrow_phase_process_mesh_plane_contacts_kernel(
    writer_func: Any,
    ...
    @wp.kernel(enable_backward=False, module="unique")
    def sap_narrow_phase_process_mesh_plane_contacts_kernel(
        shape_data: wp.array(dtype=wp.vec4),
        shape_transform: wp.array(dtype=wp.transform),
        shape_source: wp.array(dtype=wp.uint64),
        shape_gap: wp.array(dtype=float),
    ...
    if not reduce_contacts:
        return sap_narrow_phase_process_mesh_plane_contacts_kernel

    @wp.kernel(enable_backward=False, module="unique")
    def sap_narrow_phase_process_mesh_plane_contacts_reduce_kernel(
        shape_data: wp.array(dtype=wp.vec4),
        shape_transform: wp.array(dtype=wp.transform),
        shape_source: wp.array(dtype=wp.uint64),
        shape_gap: wp.array(dtype=float),
    ...
    return sap_narrow_phase_process_mesh_plane_contacts_reduce_kernel
...
@wp.kernel(enable_backward=False)
def sap_verify_narrow_phase_buffers(
...
class SapNarrowPhase:
    ...
    def __init__(
        ...
        *,
        ...
        device: SapDeviceLike | None = None,
        ...
        """
        Initialize SapNarrowPhase with pre-allocated buffers.
        ...
        Args:
            ...
            shape_voxel_resolution: Optional per-shape voxel resolution array used for mesh/SDF contact processing.
            contact_writer_warp_func: Optional custom contact writer function (first arg: SapContactData, second arg: custom struct type)
            has_meshes: Whether the scene contains any mesh shapes (SapGeoType.MESH). When False, mesh-related
            ...
            has_heightfields: Whether the scene contains any heightfield shapes (SapGeoType.HFIELD). When True,
        ...
        if has_meshes and not is_gpu_device:
            warnings.warn(
                "SapNarrowPhase running on CPU: mesh-mesh contacts will be skipped "
        ...
        if contact_writer_warp_func is None:
            writer_func = sap_write_contact_simple
        else:
        ...
        self.primitive_kernel = sap_create_narrow_phase_primitive_kernel(writer_func)
        ...
        if use_lean_gjk_mpr:
            ...
            self.narrow_phase_kernel = sap_create_narrow_phase_kernel_gjk_mpr(
                self.external_aabb,
                writer_func,
                support_func=sap_support_map_lean,
                post_process_contact=sap_post_process_minkowski_only,
            )
        else:
            self.narrow_phase_kernel = sap_create_narrow_phase_kernel_gjk_mpr(self.external_aabb, writer_func)
        # Create triangle contacts kernel when meshes or heightfields are present
        if has_meshes or has_heightfields:
            self.mesh_triangle_contacts_kernel = sap_create_narrow_phase_process_mesh_triangle_contacts_kernel(writer_func)
        else:
        ...
        if has_meshes:
            ...
            if self.reduce_contacts:
                self.mesh_plane_contacts_kernel = sap_create_narrow_phase_process_mesh_plane_contacts_kernel(
                    sap_write_contact_to_reducer,
            ...
            if is_gpu_device:
                if self.reduce_contacts:
                    self.mesh_mesh_contacts_kernel = sap_create_narrow_phase_process_mesh_mesh_contacts_kernel(
                        sap_write_contact_to_reducer,
        ...
        if self.reduce_contacts and has_meshes:
            ...
            self.export_reduced_contacts_kernel = sap_create_export_reduced_contacts_kernel(writer_func)
            # Global contact reducer for all mesh contact types
            self.global_contact_reducer = SapGlobalContactReducer(max_triangle_pairs, device=device)
        else:
    ...
    def launch_custom_write(
        ...
        *,
        ...
        shape_sdf_index: wp.array(dtype=wp.int32, ndim=1),  # Per-shape index into texture_sdf_data (-1 for none)
        ...
        shape_flags: wp.array(dtype=wp.int32, ndim=1),  # per-shape flags
        ...
        texture_sdf_data: wp.array(dtype=SapTextureSDFData, ndim=1) | None = None,  # Compact texture SDF data table
        shape_heightfield_index: wp.array(dtype=wp.int32, ndim=1) | None = None,
        heightfield_data: wp.array(dtype=SapHeightfieldData, ndim=1) | None = None,
        heightfield_elevations: wp.array(dtype=wp.float32, ndim=1) | None = None,
        writer_data: Any,
        device: SapDeviceLike | None = None,  # Device to launch on
    ) -> None:
        """
        ...
        Args:
            ...
            texture_sdf_data: Compact array of SapTextureSDFData structs
            ...
            shape_flags: Array of shape flags for each shape.
        ...
        if self.has_meshes or self.has_heightfields:
            ...
            second_dim = self.tile_size_mesh_convex if SAP_ENABLE_TILE_BVH_QUERY else 1
            wp.launch(
                kernel=sap_narrow_phase_find_mesh_triangle_overlaps_kernel,
            ...
            if self.reduce_contacts:
                ...
                if self.has_meshes:
                    wp.launch(
                        kernel=sap_compute_mesh_plane_block_offsets,
                        dim=1,
                        inputs=[
                            self.shape_pairs_mesh_plane,
                            self.shape_pairs_mesh_plane_count,
                ...
                wp.launch(
                    kernel=sap_mesh_triangle_contacts_to_reducer_kernel,
                    dim=self.total_num_threads,
                    inputs=[
                        shape_types,
                        shape_data,
            ...
            if self.reduce_contacts:
                wp.launch(
                    kernel=sap_reduce_buffered_contacts_kernel,
                    dim=self.total_num_threads,
                    inputs=[
                        reducer_data,
                        shape_transform,
            ...
            if wp.get_device(device).is_cuda and self.mesh_mesh_contacts_kernel is not None:
                if self.reduce_contacts and self.mesh_mesh_block_offsets is not None:
                    ...
                    wp.launch(
                        kernel=sap_compute_mesh_mesh_block_offsets,
                        dim=1,
                        inputs=[
                            self.shape_pairs_mesh_mesh,
                            self.shape_pairs_mesh_mesh_count,
        ...
        wp.launch(
            kernel=sap_verify_narrow_phase_buffers,
            dim=[1],
            inputs=[
                candidate_pair_count,
    ...
    def launch(
        ...
        *,
        ...
        shape_transform: wp.array(dtype=wp.transform, ndim=1),  # In world space
        shape_source: wp.array(dtype=wp.uint64, ndim=1),  # The index into the source array, type define by shape_types
        shape_sdf_index: wp.array(dtype=wp.int32, ndim=1)
        ...
        **kwargs: Any,
        ...
        if kwargs:
            ...
            if len(unknown_keys) == 1:
                raise TypeError(f"SapNarrowPhase.launch() got an unexpected keyword argument '{unknown_keys[0]}'")
            unknown = ", ".join(unknown_keys)
            raise TypeError(f"SapNarrowPhase.launch() got unexpected keyword arguments: {unknown}")
        ...
        if shape_collision_aabb_lower is None or shape_collision_aabb_upper is None:
            raise TypeError(
                "SapNarrowPhase.launch() missing required AABB bounds: provide either "
        ...
        # Create SapContactWriterData struct
        writer_data = SapContactWriterData()
