...
@wp.func
def sap_binary_search(values: wp.array(dtype=Any), value: Any, lower: int, upper: int) -> int:
    while lower < upper:
        mid = (lower + upper) >> 1
        if values[mid] > value:
            upper = mid
...
@wp.func
def _sap_vec2i_less(p: wp.vec2i, q: wp.vec2i) -> bool:
...
@wp.func
def _sap_vec2i_equal(p: wp.vec2i, q: wp.vec2i) -> bool:
...
def sap_is_pair_excluded(
    ...
    while low <= high:
        mid = (low + high) >> 1
        m = filter_pairs[mid]
        if _sap_vec2i_equal(pair, m):
            return True
        if _sap_vec2i_less(pair, m):
            high = mid - 1
        else:
            low = mid + 1
    return False
...
# Collision filtering
@wp.func
def sap_test_group_pair(group_a: int, group_b: int) -> bool:
...
@wp.func
def sap_test_world_and_group_pair(world_a: int, world_b: int, collision_group_a: int, collision_group_b: int) -> bool:
    ...
    return sap_test_group_pair(collision_group_a, collision_group_b)


def sap_precompute_world_map(shape_world: np.ndarray | list[int], shape_flags: np.ndarray | list[int] | None = None):
    ...
    if shape_flags is not None:
        ...
        colliding_mask = (shape_flags & SapShapeFlags.COLLIDE_SHAPES) != 0
    else:
