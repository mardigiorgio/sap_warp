"""Core narrow-phase kernels for the Newton-compatible collision front end.
...
from .broad_phase_common import sap_binary_search
...
SAP_ENABLE_MULTI_CONTACT = True

# Configuration flag for tiled BVH queries (experimental)
SAP_ENABLE_TILE_BVH_QUERY = True

# Type definitions for multi-contact manifolds
_sap_mat53f = wp.types.matrix((5, 3), wp.float32)

# Type definitions for single-contact mode
_sap_vec1 = wp.types.vector(1, wp.float32)


@wp.func
def sap_is_discrete_shape(shape_type: int) -> bool:
    """A discrete shape can be represented with a finite amount of flat polygon faces."""
    return (
        shape_type == SapGeoType.BOX
        or shape_type == SapGeoType.CONVEX_MESH
        or shape_type == SapGeoTypeEx.TRIANGLE
        or shape_type == SapGeoType.PLANE
    )


@wp.func
def sap_project_point_onto_plane(point: wp.vec3, plane_point: wp.vec3, plane_normal: wp.vec3) -> wp.vec3:
    """
...
@wp.func
def sap_compute_plane_normal_from_contacts(
    points: _sap_mat53f,
    normal: wp.vec3,
    signed_distances: SapVec5,
    count: int,
) -> wp.vec3:
    """
...
@wp.func
def sap_no_post_process_contact(
    contact_data: SapContactData,
    shape_a: SapGenericShapeData,
    pos_a_adjusted: wp.vec3,
    rot_a: wp.quat,
    shape_b: SapGenericShapeData,
...
@wp.func
def sap_post_process_minkowski_only(
    contact_data: SapContactData,
    shape_a: SapGenericShapeData,
    pos_a_adjusted: wp.vec3,
    rot_a: wp.quat,
    ...
    if type_a == SapGeoType.SPHERE or type_a == SapGeoType.CAPSULE:
    ...
    if type_b == SapGeoType.SPHERE or type_b == SapGeoType.CAPSULE:
...
@wp.func
def sap_post_process_axial_on_discrete_contact(
    contact_data: SapContactData,
    shape_a: SapGenericShapeData,
    pos_a_adjusted: wp.vec3,
    rot_a: wp.quat,
    ...
    is_discrete_a = sap_is_discrete_shape(type_a)
    is_discrete_b = sap_is_discrete_shape(type_b)
    is_axial_a = type_a == SapGeoType.CYLINDER or type_a == SapGeoType.CONE
    is_axial_b = type_b == SapGeoType.CYLINDER or type_b == SapGeoType.CONE
    ...
    if (is_discrete_a and is_axial_b) or (is_discrete_b and is_axial_a):
        ...
        if is_discrete_a and is_axial_b:
            ...
            is_cone = type_b == SapGeoType.CONE
            ...
            is_cone = type_a == SapGeoType.CONE
        ...
        if is_rolling:
            ...
            projected_point = sap_project_point_onto_plane(
...
def sap_create_compute_gjk_mpr_contacts(
    writer_func: Any,
    post_process_contact: Any = sap_post_process_axial_on_discrete_contact,
    support_func: Any = None,
):
    """
    Factory function to create a sap_compute_gjk_mpr_contacts function with a specific writer function.

    Args:
        writer_func: Function to write contact data (signature: (SapContactData, writer_data) -> None)
        post_process_contact: Function to post-process contact data
        support_func: Support mapping function (defaults to sap_support_map)

    Returns:
        A sap_compute_gjk_mpr_contacts function with the writer function baked in
    """
    if support_func is None:
        support_func = sap_support_map

    @wp.func
    def sap_compute_gjk_mpr_contacts(
        shape_a_data: SapGenericShapeData,
        shape_b_data: SapGenericShapeData,
        rot_a: wp.quat,
        rot_b: wp.quat,
        pos_a_adjusted: wp.vec3,
        pos_b_adjusted: wp.vec3,
        ...
        # Pre-pack SapContactData template with static information
        contact_template = SapContactData()
        ...
        if wp.static(SAP_ENABLE_MULTI_CONTACT):
            wp.static(sap_create_solve_convex_multi_contact(support_func, writer_func, post_process_contact))(
                shape_a_data,
                shape_b_data,
                rot_a,
                rot_b,
                ...
                type_a == SapGeoType.SPHERE
                or type_b == SapGeoType.SPHERE
                or type_a == SapGeoType.ELLIPSOID
                or type_b == SapGeoType.ELLIPSOID,
                writer_data,
                contact_template,
            )
        else:
            wp.static(sap_create_solve_convex_single_contact(support_func, writer_func, post_process_contact))(
                shape_a_data,
                shape_b_data,
                rot_a,
                rot_b,
    ...
    return sap_compute_gjk_mpr_contacts


@wp.func
def sap_compute_tight_aabb_from_support(
    ...
    if shape_data.shape_type == SapGeoType.CONVEX_MESH:
        # Single-pass AABB: iterate over vertices once, project onto all 3 axes.
        # This replaces 6 separate sap_support_map calls (each iterating all vertices)
        # with 1 pass that computes min/max projections simultaneously.
        mesh_ptr = sap_unpack_mesh_ptr(shape_data.auxiliary)
        ...
        support_point = sap_support_map(shape_data, local_x, data_provider)
        max_x = wp.dot(local_x, support_point)

        support_point = sap_support_map(shape_data, local_y, data_provider)
        max_y = wp.dot(local_y, support_point)

        support_point = sap_support_map(shape_data, local_z, data_provider)
...
@wp.func
def sap_compute_bounding_sphere_from_aabb(aabb_lower: wp.vec3, aabb_upper: wp.vec3) -> tuple[wp.vec3, float]:
    """
...
@wp.func
def sap_convert_infinite_plane_to_cube(
    shape_data: SapGenericShapeData,
    ...
    Args:
        shape_data: The plane's shape data (should have shape_type == SapGeoType.PLANE)
    ...
    Returns:
        ...
        - modified_shape_data: SapGenericShapeData configured as a BOX
        - adjusted_position: The cube's center position (centered on other object projected to plane)
    """
    result = SapGenericShapeData()
    result.shape_type = SapGeoType.BOX
...
@wp.func
def sap_check_infinite_plane_bsphere_overlap(
    shape_data_a: SapGenericShapeData,
    shape_data_b: SapGenericShapeData,
    pos_a: wp.vec3,
    pos_b: wp.vec3,
    quat_a: wp.quat,
    quat_b: wp.quat,
...
def sap_create_find_contacts(writer_func: Any, support_func: Any = None, post_process_contact: Any = None):
    """
    Factory function to create a sap_find_contacts function with a specific writer function.

    Args:
        writer_func: Function to write contact data (signature: (SapContactData, writer_data) -> None)
        support_func: Support mapping function (defaults to sap_support_map)
        post_process_contact: Post-processing function (defaults to sap_post_process_axial_on_discrete_contact)

    Returns:
        A sap_find_contacts function with the writer function baked in
    """
    if support_func is None:
        support_func = sap_support_map
    if post_process_contact is None:
        post_process_contact = sap_post_process_axial_on_discrete_contact

    @wp.func
    def sap_find_contacts(
        pos_a: wp.vec3,
        pos_b: wp.vec3,
        quat_a: wp.quat,
        quat_b: wp.quat,
        ...
        if is_infinite_plane_a:
            ...
            # Note: sap_convert_infinite_plane_to_cube modifies shape_data_a.shape_type to BOX
            shape_data_a, pos_a_adjusted = sap_convert_infinite_plane_to_cube(
        ...
        if is_infinite_plane_b:
            ...
            # Note: sap_convert_infinite_plane_to_cube modifies shape_data_b.shape_type to BOX
            shape_data_b, pos_b_adjusted = sap_convert_infinite_plane_to_cube(
    ...
    return sap_find_contacts


@wp.func
def sap_pre_contact_check(
    shape_a: int,
    shape_b: int,
    pos_a: wp.vec3,
    pos_b: wp.vec3,
    ...
    is_infinite_plane_a = (type_a == SapGeoType.PLANE) and (shape_data_a.scale[0] == 0.0 and shape_data_a.scale[1] == 0.0)
    is_infinite_plane_b = (type_b == SapGeoType.PLANE) and (shape_data_b.scale[0] == 0.0 and shape_data_b.scale[1] == 0.0)
    ...
    if type_a == SapGeoType.PLANE and type_b == SapGeoType.MESH:
    ...
    if type_a == SapGeoType.MESH or type_b == SapGeoType.MESH:
...
@wp.func
def sap_mesh_vs_convex_midphase(
    ...
    generic_shape_data = SapGenericShapeData()
    ...
    if geo_type == SapGeoType.CONVEX_MESH:
        generic_shape_data.auxiliary = sap_pack_mesh_ptr(shape_source_ptr[non_mesh_shape])

    data_provider = SapSupportMapDataProvider()

    # Compute tight AABB directly in mesh local space for optimal fit
    aabb_lower, aabb_upper = sap_compute_tight_aabb_from_support(
    ...
    if wp.static(SAP_ENABLE_TILE_BVH_QUERY):
...
@wp.func
def sap_find_pair_from_cumulative_index(
    global_idx: int,
    ...
    """
    # Use sap_binary_search to find first index where cumulative_sums[i] > global_idx
    # This gives us the bucket that contains global_idx
    pair_idx = sap_binary_search(cumulative_sums, global_idx, 0, pair_count)
...
@wp.func
def sap_get_triangle_shape_from_mesh(
    mesh_id: wp.uint64,
    mesh_scale: wp.vec3,
    X_mesh_ws: wp.transform,
    tri_idx: int,
    ...
    This function retrieves a specific triangle from a mesh and creates a SapGenericShapeData
    ...
    Returns:
        ...
        - shape_data: SapGenericShapeData with triangle geometry (type=TRIANGLE, scale=B-A, auxiliary=C-A)
...
@wp.func
def sap_get_box_axes(q: wp.quat) -> wp.mat33:
...
@wp.func
def sap_project_box_onto_axis(transform: wp.transform, extents: wp.vec3, axis: wp.vec3) -> wp.vec2:
    """Project a box onto an axis and return [min, max] projection values"""
    # Get box axes and extents
    axes = sap_get_box_axes(wp.transform_get_rotation(transform))
...
def sap_test_axis_separation(
    ...
    proj_a = sap_project_box_onto_axis(transform_a, extents_a, normalized_axis)
    proj_b = sap_project_box_onto_axis(transform_b, extents_b, normalized_axis)
...
def sap_sat_box_intersection(
    ...
    axes_a = sap_get_box_axes(wp.transform_get_rotation(transform_a))
    axes_b = sap_get_box_axes(wp.transform_get_rotation(transform_b))

    # Test the 15 potential separating axes

    # Test face normals of box A (3 axes)
    for i in range(3):
        if sap_test_axis_separation(transform_a, extents_a, transform_b, extents_b, axes_a[i]):
            return False  # Boxes are separated

    # Test face normals of box B (3 axes)
    for i in range(3):
        if sap_test_axis_separation(transform_a, extents_a, transform_b, extents_b, axes_b[i]):
            return False  # Boxes are separated
    ...
    for i in range(3):
        for j in range(3):
            ...
            if sap_test_axis_separation(transform_a, extents_a, transform_b, extents_b, cross_axis):
                return False  # Boxes are separated
