...
Source note: the SAP modifications in this module are based on Newton's GJK
collision code and adapted for SAP Warp's narrow-phase stages.
...
from .mpr import SapMprVert, sap_create_support_map_function

SAP_SIMPLEX_EPSILON = 1e-8

SapMat83f = wp.types.matrix(shape=(8, 3), dtype=wp.float32)


def sap_create_solve_closest_distance(support_func: Any, _support_funcs: Any = None):
    """
    Factory function to create GJK distance solver with specific support and center functions.

    Storage Convention for Simplex Vertices (SapMat83f):
    ------------------------------------------------
    ...
    Args:
        ...
        _support_funcs: Pre-built support functions tuple from
            :func:`sap_create_support_map_function`. When provided, these are reused
            instead of creating new ones, allowing multiple solvers to share
            compiled support code.

    Returns:
        ``sap_solve_closest_distance`` wrapper function.  The core function is
        available as ``sap_solve_closest_distance.core`` for callers that want to
    ...
    if _support_funcs is not None:
        ...
        _support_map_b, minkowski_support, geometric_center = sap_create_support_map_function(support_func)

    @wp.func
    def sap_simplex_get_vertex(v: SapMat83f, i: int) -> SapMprVert:
        """
    ...
    def sap_closest_segment(
        ...
        degenerate = vsq < SAP_SIMPLEX_EPSILON

        # Guard division by zero in degenerate cases
        denom = vsq
        if degenerate:
            denom = SAP_SIMPLEX_EPSILON
    ...
    def sap_closest_triangle(
        ...
        degenerate = t < SAP_SIMPLEX_EPSILON
        # Guard division by zero in degenerate cases
        denom = t
        if degenerate:
            denom = SAP_SIMPLEX_EPSILON
        ...
        if lambda0 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_segment(v, i1, i2)
        ...
        if lambda1 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_segment(v, i0, i2)
            dist = wp.length_sq(closest)
            if dist < best_distance:
                bc = bc_tmp
                mask = m
        ...
        if lambda2 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_segment(v, i0, i1)
            dist = wp.length_sq(closest)
            if dist < best_distance:
                bc = bc_tmp
                mask = m
    ...
    @wp.func
    def sap_determinant(a: wp.vec3, b: wp.vec3, c: wp.vec3, d: wp.vec3) -> float:
        """Compute sap_determinant for tetrahedron volume."""
        return wp.dot(b - a, wp.cross(c - a, d - a))

    @wp.func
    def sap_closest_tetrahedron(
        v: SapMat83f,
    ) -> tuple[wp.vec3, wp.vec4, wp.uint32]:
        ...
        det_t = sap_determinant(v0, v1, v2, v3)
        degenerate = wp.abs(det_t) < SAP_SIMPLEX_EPSILON
        # Guard division by zero in degenerate cases
        denom = det_t
        if degenerate:
            denom = SAP_SIMPLEX_EPSILON
        ...
        lambda0 = sap_determinant(zero, v1, v2, v3) * inverse_det_t
        lambda1 = sap_determinant(v0, zero, v2, v3) * inverse_det_t
        lambda2 = sap_determinant(v0, v1, zero, v3) * inverse_det_t
        ...
        # Check faces
        if lambda0 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_triangle(v, 1, 2, 3)
            dist = wp.length_sq(closest)
            if dist < best_distance:
                bc = bc_tmp
                mask = m
        ...
        if lambda1 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_triangle(v, 0, 2, 3)
            dist = wp.length_sq(closest)
            if dist < best_distance:
                bc = bc_tmp
                mask = m
        ...
        if lambda2 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_triangle(v, 0, 1, 3)
            dist = wp.length_sq(closest)
            if dist < best_distance:
                bc = bc_tmp
                mask = m
        ...
        if lambda3 < 0.0 or degenerate:
            closest, bc_tmp, m = sap_closest_triangle(v, 0, 1, 2)
            dist = wp.length_sq(closest)
            if dist < best_distance:
                bc = bc_tmp
                mask = m
    ...
    @wp.func
    def sap_simplex_get_closest(v: SapMat83f, barycentric: wp.vec4, usage_mask: wp.uint32) -> tuple[wp.vec3, wp.vec3]:
        ...
        for i in range(4):
            ...
            vertex = sap_simplex_get_vertex(v, i)
    ...
    @wp.func
    def sap_solve_closest_distance_core(
        geom_a: Any,
        geom_b: Any,
        ...
        while iter_count > 0:
            ...
            if dist_sq < COLLIDE_EPSILON * COLLIDE_EPSILON:
                ...
                distance = 0.0
                normal = wp.vec3(0.0, 0.0, 0.0)
                point_a, point_b = sap_simplex_get_closest(simplex_v, simplex_barycentric, simplex_usage_mask)
            ...
            if use_count == 1:
                ...
                i0 = indices[0]
                i1 = indices[1]
                closest, bc, mask = sap_closest_segment(simplex_v, i0, i1)
                ...
                i1 = indices[1]
                i2 = indices[2]
                closest, bc, mask = sap_closest_triangle(simplex_v, i0, i1, i2)
                simplex_barycentric = bc
                simplex_usage_mask = mask
            elif use_count == 4:
                closest, bc, mask = sap_closest_tetrahedron(simplex_v)
                simplex_barycentric = bc
                simplex_usage_mask = mask
        ...
        if delta_len_sq > SAP_SIMPLEX_EPSILON * SAP_SIMPLEX_EPSILON:
    ...
    @wp.func
    def sap_solve_closest_distance(
        geom_a: Any,
        geom_b: Any,
        ...
        result = sap_solve_closest_distance_core(
            geom_a,
            geom_b,
    ...
    sap_solve_closest_distance.core = sap_solve_closest_distance_core
    return sap_solve_closest_distance
