"""Global contact reduction kernels for the Newton-compatible collision path.

Source note: the SAP modifications in this module are based on Newton's
collision reduction code and adapted for SAP Warp contact buffers.
"""
...
from .core import sap_create_compute_gjk_mpr_contacts, sap_get_triangle_shape_from_mesh
from .hashtable import SAP_HASHTABLE_EMPTY_KEY, SapHashTable, sap_hashtable_find_or_insert
from .heightfield import SapHeightfieldData, sap_get_triangle_shape_from_heightfield
...
from .support import sap_extract_shape_data
...
# the support polygon.
SAP_GLOBAL_REDUCTION_BETA_THRESHOLD = 0.0001  # 0.1mm

# Number of value slots per hashtable entry: 6 spatial directions + 1 max-depth = 7
SAP_REDUCTION_VALUES_PER_KEY = SAP_NUM_SPATIAL_DIRECTIONS + 1

# Vector type for tracking exported contact IDs (used in export kernels)
SapExportedIdsVec = wp.types.vector(length=SAP_REDUCTION_VALUES_PER_KEY, dtype=wp.int32)


@wp.func
def sap_is_contact_already_exported(
    contact_id: int,
    exported_ids: wp.types.vector(length=SAP_REDUCTION_VALUES_PER_KEY, dtype=wp.int32),
    num_exported: int,
) -> bool:
...
@wp.func
def sap_compute_effective_radius(shape_type: int, shape_scale: wp.vec4) -> float:
...
def sap_reduction_update_slot(
    ...
    Use this after sap_hashtable_find_or_insert() to write multiple values
    to the same entry without repeated hash lookups.

    Args:
        entry_idx: Entry index from sap_hashtable_find_or_insert()
...
def sap_reduction_insert_slot(
    ...
    Convenience function that combines sap_hashtable_find_or_insert()
    and sap_reduction_update_slot(). For inserting multiple values to
    ...
    entry_idx = sap_hashtable_find_or_insert(key, keys, active_slots)
    if entry_idx < 0:
        return False
    sap_reduction_update_slot(entry_idx, slot_id, value, values, capacity)
    return True
...
SAP_SHAPE_A_BITS = wp.constant(wp.uint64(27))
SAP_SHAPE_A_MASK = wp.constant(wp.uint64((1 << 27) - 1))
SAP_SHAPE_B_BITS = wp.constant(wp.uint64(28))
SAP_SHAPE_B_MASK = wp.constant(wp.uint64((1 << 28) - 1))
SAP_BIN_BITS = wp.constant(wp.uint64(8))
SAP_BIN_MASK = wp.constant(wp.uint64((1 << 8) - 1))


@wp.func
def sap_reduction_make_contact_key(shape_a: int, shape_b: int, bin_id: int) -> wp.uint64:
    ...
    """
    key = wp.uint64(shape_a) & SAP_SHAPE_A_MASK
    key = key | ((wp.uint64(shape_b) & SAP_SHAPE_B_MASK) << SAP_SHAPE_A_BITS)
    # bin_id goes at bits 55-62 (after 27 + 28 = 55 bits for shape IDs)
    key = key | ((wp.uint64(bin_id) & SAP_BIN_MASK) << wp.uint64(55))
    return key


@wp.func
def sap_reduction_make_contact_value(score: float, contact_id: int) -> wp.uint64:
    """Pack score and contact_id into hashtable value for atomic max.

    High 32 bits: sap_float_flip(score) - makes floats comparable as unsigned ints
    ...
    """
    return (wp.uint64(sap_float_flip(score)) << wp.uint64(32)) | wp.uint64(contact_id)


@wp.func_native("""
return static_cast<int32_t>(packed & 0xFFFFFFFFull);
""")
def sap_reduction_unpack_contact_id(packed: wp.uint64) -> int:
    """Extract contact_id from packed value."""
    ...


@wp.func
def sap_reduction_encode_oct(n: wp.vec3) -> wp.vec2:
...
@wp.func
def sap_reduction_decode_oct(e: wp.vec2) -> wp.vec3:
    """Decode octahedral 2D representation back to a unit normal.

    Inverse of sap_reduction_encode_oct.  Lossless within float precision.
    """
...
@wp.struct
class SapGlobalContactReducerData:
    """Struct for passing SapGlobalContactReducer arrays to kernels.
    ...
    normal: wp.array(dtype=wp.vec2)  # Octahedral-encoded unit normal (see sap_reduction_encode_oct/sap_reduction_decode_oct)
...
def _sap_clear_active_kernel(
    ...
    while i < total_work:
        ...
        if local_idx == 0:
            ht_keys[entry_idx] = SAP_HASHTABLE_EMPTY_KEY
...
@wp.kernel
def _sap_zero_count_and_contacts_kernel(
    ht_active_slots: wp.array(dtype=wp.int32),
    contact_count: wp.array(dtype=wp.int32),
    ht_insert_failures: wp.array(dtype=wp.int32),
    ht_capacity: int,
...
class SapGlobalContactReducer:
    ...
    **Contact Data Storage:**
    ...
    Attributes:
        ...
        hashtable: SapHashTable for tracking best contacts (keys only)
        ht_values: Values array for hashtable (managed here, not by SapHashTable)
    """
    ...
    def __init__(
        ...
        self.values_per_key = SAP_NUM_SPATIAL_DIRECTIONS + 1
        ...
        self.hashtable = SapHashTable(hashtable_size, device=device)

        # Values array for hashtable - managed here, not by SapHashTable
    ...
    def clear_active(self):
        ...
        wp.launch(
            _sap_zero_count_and_contacts_kernel,
            dim=1,
            inputs=[
                self.hashtable.active_slots,
                self.contact_count,
    ...
    def get_data_struct(self) -> SapGlobalContactReducerData:
        """Get a SapGlobalContactReducerData struct for passing to kernels.

        Returns:
            A SapGlobalContactReducerData struct containing all arrays.
        """
        data = SapGlobalContactReducerData()
...
@wp.func
def sap_export_contact_to_buffer(
    shape_a: int,
    shape_b: int,
    position: wp.vec3,
    normal: wp.vec3,
    depth: float,
    reducer_data: SapGlobalContactReducerData,
) -> int:
    ...
    Args:
        ...
        reducer_data: SapGlobalContactReducerData with all arrays
    ...
    reducer_data.normal[contact_id] = sap_reduction_encode_oct(normal)
...
@wp.func
def sap_reduce_contact_in_hashtable(
    contact_id: int,
    reducer_data: SapGlobalContactReducerData,
    beta: float,
    ...
    normal = sap_reduction_decode_oct(reducer_data.normal[contact_id])
    ...
    bin_id = sap_get_slot(normal)

    # Project position to 2D plane of the icosahedron face
    pos_2d = sap_project_point_to_plane(bin_id, position)

    # Key is (shape_a, shape_b, bin_id)
    key = sap_reduction_make_contact_key(shape_a, shape_b, bin_id)

    # Find or create the hashtable entry ONCE, then write directly to slots
    entry_idx = sap_hashtable_find_or_insert(key, reducer_data.ht_keys, reducer_data.ht_active_slots)
    if entry_idx >= 0:
        ...
        for dir_i in range(SAP_NUM_SPATIAL_DIRECTIONS):
            if use_beta:
                dir_2d = sap_get_spatial_direction_2d(dir_i)
                score = wp.dot(pos_2d, dir_2d)
                value = sap_reduction_make_contact_value(score, contact_id)
                slot_id = dir_i
                sap_reduction_update_slot(entry_idx, slot_id, value, reducer_data.ht_values, ht_capacity)

        # Also register for max-depth slot (last slot = 6)
        # Use -depth as score so atomic_max selects the deepest (most negative depth)
        max_depth_slot_id = SAP_NUM_SPATIAL_DIRECTIONS  # = 6
        max_depth_value = sap_reduction_make_contact_value(-depth, contact_id)
        sap_reduction_update_slot(entry_idx, max_depth_slot_id, max_depth_value, reducer_data.ht_values, ht_capacity)
    else:
    ...
    voxel_idx = sap_compute_voxel_index(position_local, aabb_lower, aabb_upper, voxel_res)

    # Clamp voxel index to valid range
    voxel_idx = wp.clamp(voxel_idx, 0, wp.static(SAP_NUM_VOXEL_DEPTH_SLOTS - 1))

    # Group voxels by 7 to maximize slot utilization (matches values_per_key)
    # 100 voxels -> 15 hashtable entries (ceil(100/7) = 15)
    # bin_id = SAP_NUM_NORMAL_BINS + voxel_group (20-34)
    # slot = voxel_local (0-6)
    voxels_per_group = wp.static(SAP_NUM_SPATIAL_DIRECTIONS + 1)  # = 7 (same as values_per_key)
    voxel_group = voxel_idx // voxels_per_group
    voxel_local_slot = voxel_idx % voxels_per_group

    voxel_bin_id = SAP_NUM_NORMAL_BINS + voxel_group
    voxel_key = sap_reduction_make_contact_key(shape_a, shape_b, voxel_bin_id)

    voxel_entry_idx = sap_hashtable_find_or_insert(voxel_key, reducer_data.ht_keys, reducer_data.ht_active_slots)
    if voxel_entry_idx >= 0:
        # Use -depth so atomic_max selects most penetrating (most negative depth)
        voxel_value = sap_reduction_make_contact_value(-depth, contact_id)
        sap_reduction_update_slot(voxel_entry_idx, voxel_local_slot, voxel_value, reducer_data.ht_values, ht_capacity)
    else:
        wp.atomic_add(reducer_data.ht_insert_failures, 0, 1)


@wp.func
def sap_export_and_reduce_contact(
    shape_a: int,
    shape_b: int,
    position: wp.vec3,
    normal: wp.vec3,
    ...
    contact_id = sap_export_contact_to_buffer(shape_a, shape_b, position, normal, depth, reducer_data)
...
@wp.func
def sap_export_and_reduce_contact_centered(
    shape_a: int,
    shape_b: int,
    position: wp.vec3,
    normal: wp.vec3,
    ...
    Differs from :func:`sap_export_and_reduce_contact` in three ways that match
    ...
    """
    ht_capacity = reducer_data.ht_capacity
    use_beta = depth < wp.static(SAP_GLOBAL_REDUCTION_BETA_THRESHOLD)

    # === Normal bin: find/create hashtable entry ===
    bin_id = sap_get_slot(normal)
    pos_2d = sap_project_point_to_plane(bin_id, centered_position)
    key = sap_reduction_make_contact_key(shape_a, shape_b, bin_id)
    ...
    if entry_idx >= 0:
        ...
        max_depth_probe = sap_reduction_make_contact_value(-depth, 0)
        if reducer_data.ht_values[SAP_NUM_SPATIAL_DIRECTIONS * ht_capacity + entry_idx] < max_depth_probe:
            might_win = True
        ...
        if not might_win and use_beta:
            for dir_i in range(SAP_NUM_SPATIAL_DIRECTIONS):
                if not might_win:
                    dir_2d = sap_get_spatial_direction_2d(dir_i)
                    score = wp.dot(pos_2d, dir_2d)
                    probe = sap_reduction_make_contact_value(score, 0)
    ...
    voxel_idx = sap_compute_voxel_index(position_local, aabb_lower_voxel, aabb_upper_voxel, voxel_res)
    voxel_idx = wp.clamp(voxel_idx, 0, wp.static(SAP_NUM_VOXEL_DEPTH_SLOTS - 1))

    voxels_per_group = wp.static(SAP_NUM_SPATIAL_DIRECTIONS + 1)
    voxel_group = voxel_idx // voxels_per_group
    voxel_local_slot = voxel_idx % voxels_per_group
    voxel_bin_id = SAP_NUM_NORMAL_BINS + voxel_group
    voxel_key = sap_reduction_make_contact_key(shape_a, shape_b, voxel_bin_id)
    ...
    if not might_win:
        voxel_entry_idx = sap_hashtable_find_or_insert(voxel_key, reducer_data.ht_keys, reducer_data.ht_active_slots)
        if voxel_entry_idx >= 0:
            voxel_probe = sap_reduction_make_contact_value(-depth, 0)
    ...
    if entry_idx >= 0:
        for dir_i in range(SAP_NUM_SPATIAL_DIRECTIONS):
            if use_beta:
                ...
                score = wp.dot(pos_2d, dir_2d)
                value = sap_reduction_make_contact_value(score, contact_id)
                sap_reduction_update_slot(entry_idx, dir_i, value, reducer_data.ht_values, ht_capacity)

        max_depth_value = sap_reduction_make_contact_value(-depth, contact_id)
        sap_reduction_update_slot(entry_idx, SAP_NUM_SPATIAL_DIRECTIONS, max_depth_value, reducer_data.ht_values, ht_capacity)
    else:
        wp.atomic_add(reducer_data.ht_insert_failures, 0, 1)
...
def sap_reduce_buffered_contacts_kernel(
    reducer_data: SapGlobalContactReducerData,
    shape_transform: wp.array(dtype=wp.transform),
    shape_collision_aabb_lower: wp.array(dtype=wp.vec3),
    shape_collision_aabb_upper: wp.array(dtype=wp.vec3),
    ...
    Uses the fixed SAP_GLOBAL_REDUCTION_BETA_THRESHOLD (0.1mm) for spatial competition.
    ...
    for i in range(tid, num_contacts, total_num_threads):
        sap_reduce_contact_in_hashtable(
            i,
            reducer_data,
            wp.static(SAP_GLOBAL_REDUCTION_BETA_THRESHOLD),
            shape_transform,
            shape_collision_aabb_lower,
            shape_collision_aabb_upper,
            shape_voxel_resolution,
...
def sap_unpack_contact(
    ...
    n = sap_reduction_decode_oct(normal[contact_id])
...
@wp.func
def sap_write_contact_to_reducer(
    contact_data: Any,  # SapContactData struct
    reducer_data: SapGlobalContactReducerData,
    output_index: int,  # Unused, kept for API compatibility with write_contact_simple
):
    """Writer function that stores contacts in SapGlobalContactReducer for reduction.

    This follows the same signature as write_contact_simple in narrow_phase.py,
    so it can be used with sap_create_compute_gjk_mpr_contacts and other contact
    ...
    Args:
        contact_data: SapContactData struct from contact computation
        reducer_data: SapGlobalContactReducerData struct with all reducer arrays
        output_index: Unused, kept for API compatibility
    """
    # Extract contact info from SapContactData
...
def sap_create_export_reduced_contacts_kernel(writer_func: Any):
    ...
    Each entry has SAP_REDUCTION_VALUES_PER_KEY value slots (7: 6 spatial + 1 max-depth).
    ...
    Args:
        writer_func: A warp function with signature (SapContactData, writer_data, int) -> None.
    ...
    exported_ids_vec = wp.types.vector(length=SAP_REDUCTION_VALUES_PER_KEY, dtype=wp.int32)

    @wp.kernel(enable_backward=False, module="unique")
    def sap_export_reduced_contacts_kernel(
        # Hashtable arrays
        ht_keys: wp.array(dtype=wp.uint64),
        ht_values: wp.array(dtype=wp.uint64),
        ht_active_slots: wp.array(dtype=wp.int32),
        ...
        for i in range(tid, num_active, total_num_threads):
            ...
            for slot in range(wp.static(SAP_REDUCTION_VALUES_PER_KEY)):
                ...
                contact_id = sap_reduction_unpack_contact_id(value)

                # Skip if already exported within this entry
                if sap_is_contact_already_exported(contact_id, exported_ids, num_exported):
                    continue
                ...
                # Unpack contact data
                position, contact_normal, depth = sap_unpack_contact(contact_id, position_depth, normal)
                ...
                radius_eff_a = sap_compute_effective_radius(shape_types[shape_a], shape_data[shape_a])
                radius_eff_b = sap_compute_effective_radius(shape_types[shape_b], shape_data[shape_b])
    ...
    return sap_export_reduced_contacts_kernel


@wp.kernel(enable_backward=False, module="unique")
def sap_mesh_triangle_contacts_to_reducer_kernel(
    shape_types: wp.array(dtype=int),
    shape_data: wp.array(dtype=wp.vec4),
    shape_transform: wp.array(dtype=wp.transform),
    ...
    """Process mesh/heightfield-triangle contacts and store them in SapGlobalContactReducer.

    This kernel processes triangle pairs (mesh-or-hfield shape, convex-shape, triangle_index)
    and computes contacts using GJK/MPR, storing results in the SapGlobalContactReducer for
    ...
    for i in range(tid, num_triangle_pairs, total_num_threads):
        ...
        if type_a == SapGeoType.HFIELD:
            ...
            shape_data_a, v0_world = sap_get_triangle_shape_from_heightfield(hfd, heightfield_elevations, X_ws_a, tri_idx)
        else:
        ...
        wp.static(sap_create_compute_gjk_mpr_contacts(sap_write_contact_to_reducer))(
            shape_data_a,
            shape_data_b,
            quat_a,
            quat_b,
