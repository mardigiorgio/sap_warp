...
# Is not allowed to share values with SapGeoType
class SapGeoTypeEx(enum.IntEnum):
    TRIANGLE = 1000


@wp.struct
class SapSupportMapDataProvider:
    """Placeholder for optional support-map backing data."""

    pass


@wp.func
def sap_pack_mesh_ptr(ptr: wp.uint64) -> wp.vec3:
...
@wp.func
def sap_unpack_mesh_ptr(arr: wp.vec3) -> wp.uint64:
...
@wp.struct
class SapGenericShapeData:
    """
    Minimal shape descriptor for support mapping.

    Fields:
    - shape_type: matches values from SapGeoType
...
@wp.func
def sap_support_map(geom: SapGenericShapeData, direction: wp.vec3, data_provider: SapSupportMapDataProvider) -> wp.vec3:
    """
    ...
    if geom.shape_type == SapGeoType.CONVEX_MESH:
        # Convex hull support: find the furthest point in the direction
        mesh_ptr = sap_unpack_mesh_ptr(geom.auxiliary)
    ...
    elif geom.shape_type == SapGeoTypeEx.TRIANGLE:
        ...
        if dot_a >= dot_b and dot_a >= dot_c:
            ...
            result = tri_c
    elif geom.shape_type == SapGeoType.BOX:
    ...
    elif geom.shape_type == SapGeoType.SPHERE:
        radius = geom.scale[0]
    ...
    elif geom.shape_type == SapGeoType.CAPSULE:
        radius = geom.scale[0]
    ...
    elif geom.shape_type == SapGeoType.ELLIPSOID:
    ...
    elif geom.shape_type == SapGeoType.CYLINDER:
        radius = geom.scale[0]
        half_height = geom.scale[1]
    ...
    elif geom.shape_type == SapGeoType.CONE:
        radius = geom.scale[0]
        half_height = geom.scale[1]
    ...
    elif geom.shape_type == SapGeoType.PLANE:
...
@wp.func
def sap_support_map_lean(geom: SapGenericShapeData, direction: wp.vec3, data_provider: SapSupportMapDataProvider) -> wp.vec3:
    """
    Lean support function for common shape types only: CONVEX_MESH, BOX, SPHERE.

    This is a specialized version of sap_support_map with reduced code size to improve
...
def sap_extract_shape_data(
    ...
    if shape_types[shape_idx] == SapGeoType.CONVEX_MESH:
        result.auxiliary = sap_pack_mesh_ptr(shape_source[shape_idx])
