...
multi-contact collision code and adapted for SAP Warp's narrow-phase stages.
...
from .math import sap_orthonormal_basis
from .mpr import sap_create_support_map_function

# Constants
SAP_MULTICONTACT_EPS = 0.00001
# The tilt angle defines how much the search direction gets tilted while searching for
# points on the contact manifold.
SAP_TILT_ANGLE_RAD = wp.static(2.0 * wp.pi / 180.0)
SAP_SIN_TILT_ANGLE = wp.static(wp.sin(SAP_TILT_ANGLE_RAD))
SAP_COS_TILT_ANGLE = wp.static(wp.cos(SAP_TILT_ANGLE_RAD))

SAP_COS_DEEPEST_CONTACT_THRESHOLD_ANGLE = wp.static(wp.cos(0.1 * wp.pi / 180.0))


@wp.func
def sap_should_include_deepest_contact(normal_dot: float) -> bool:
    return normal_dot < SAP_COS_DEEPEST_CONTACT_THRESHOLD_ANGLE


@wp.func
def sap_excess_normal_deviation(dir_a: wp.vec3, dir_b: wp.vec3) -> bool:
    """
    ...
    Returns:
        True if the angle between the vectors exceeds SAP_TILT_ANGLE_RAD (2 degrees).
    """
    dot = wp.abs(wp.dot(dir_a, dir_b))
    return dot < SAP_COS_TILT_ANGLE


@wp.func
def sap_signed_area(a: wp.vec2, b: wp.vec2, query_point: wp.vec2) -> float:
    """
...
@wp.func
def sap_ray_plane_intersection(
...
@wp.struct
class SapIncrementalPlaneTracker:
...
@wp.func
def sap_update_incremental_plane_tracker(
    tracker: SapIncrementalPlaneTracker,
...
@wp.func
def sap_compute_line_segment_projector_normal(
    segment_dir: wp.vec3,
...
@wp.func
def sap_create_body_projectors(
    plane_tracker_a: SapIncrementalPlaneTracker,
    anchor_point_a: wp.vec3,
    plane_tracker_b: SapIncrementalPlaneTracker,
    anchor_point_b: wp.vec3,
    contact_normal: wp.vec3,
) -> tuple[SapBodyProjector, SapBodyProjector]:
    projector_a = SapBodyProjector()
    projector_b = SapBodyProjector()
    ...
    if plane_tracker_a.largest_area_sq == 0.0 and plane_tracker_b.largest_area_sq == 0.0:
        ...
        projector_a.normal = sap_compute_line_segment_projector_normal(dir_a, contact_normal)
        ...
        projector_b.normal = sap_compute_line_segment_projector_normal(dir_b, contact_normal)
    ...
    if plane_tracker_a.largest_area_sq == 0.0:
        ...
        point_on_plane_a = 0.5 * (plane_tracker_a.reference_point + plane_tracker_a.previous_point)
        projector_a.normal = sap_compute_line_segment_projector_normal(dir, projector_b.normal)
        projector_a.plane_d = -wp.dot(point_on_plane_a, projector_a.normal)

    if plane_tracker_b.largest_area_sq == 0.0:
        dir = plane_tracker_b.previous_point - plane_tracker_b.reference_point
        point_on_plane_b = 0.5 * (plane_tracker_b.reference_point + plane_tracker_b.previous_point)
        projector_b.normal = sap_compute_line_segment_projector_normal(dir, projector_a.normal)
        projector_b.plane_d = -wp.dot(point_on_plane_b, projector_b.normal)
...
@wp.func
def sap_body_projector_project(
    proj: SapBodyProjector,
    input: wp.vec3,
    contact_normal: wp.vec3,
) -> wp.vec3:
    """
    ...
    return sap_ray_plane_intersection(input, contact_normal, proj.plane_d, proj.normal)


@wp.func
def sap_intersection_point(trim_seg_start: wp.vec2, trim_seg_end: wp.vec2, a: wp.vec2, b: wp.vec2) -> wp.vec2:
    """
    ...
    signed_a = sap_signed_area(trim_seg_start, trim_seg_end, a)
    signed_b = sap_signed_area(trim_seg_start, trim_seg_end, b)
...
@wp.func
def sap_insert_vec2(arr: wp.array(dtype=wp.vec2), arr_count: int, index: int, element: wp.vec2):
    """
...
def sap_trim_in_place(
    ...
    # Check first vertex
    prev_outside = bool(sap_signed_area(trim_seg_start, trim_seg_end, loop[0]) <= 0.0)

    for i in range(loop_count):
        next_idx = (i + 1) % loop_count
        outside = sap_signed_area(trim_seg_start, trim_seg_end, loop[next_idx]) <= 0.0

        if outside != prev_outside:
            intersection = sap_intersection_point(trim_seg_start, trim_seg_end, loop[i], loop[next_idx])
            if change_a < 0:
                change_a = i
        ...
        while i < loop_count:
            ...
            if i == change_a or i == change_b:
                ...
                if loop_indexer == i and not keep:
                    loop_indexer += 1
                    sap_insert_vec2(loop, new_loop_count, loop_indexer, pt)
...
def sap_trim_all_in_place(
    ...
    for i in range(trim_poly_count):
        ...
        current_loop_count = sap_trim_in_place(trim_seg_start, trim_seg_end, loop, current_loop_count)

    return current_loop_count


@wp.func
def sap_approx_max_quadrilateral_area_with_calipers(hull: wp.array(dtype=wp.vec2), hull_count: int) -> wp.vec4i:
    """
    ...
    for i in range(n):
        ...
        while True:
            ...
            area_j_plus_1 = sap_signed_area(hull_i, hull_i_plus_1, hull_j_plus_1)
            area_j = sap_signed_area(hull_i, hull_i_plus_1, hull_j)
    ...
    for i in range(n):
        ...
        hull_i = hull[i]
        area = sap_signed_area(hull_p1, hull_p3, hull_i)
...
@wp.func
def sap_remove_zero_length_edges(loop: wp.array(dtype=wp.vec2), loop_count: int, eps: float) -> int:
    """
...
@wp.func
def sap_add_avoid_duplicates_vec2(
...
""")
def sap_get_ptr(a: wp.array(dtype=wp.vec2)) -> wp.uint64: ...


def sap_create_build_manifold(support_func: Any, writer_func: Any, post_process_contact: Any, _support_funcs: Any = None):
    """
    ...
    - sap_build_manifold_core: The core implementation that uses preallocated buffers
    - sap_build_manifold: The main entry point that handles buffer allocation and result extraction
    ...
    Returns:
        sap_build_manifold function that generates up to 5 contact points between two shapes
    ...
    if _support_funcs is not None:
        ...
        _support_map_b = sap_create_support_map_function(support_func)[0]

    @wp.func
    def sap_extract_4_point_contact_manifolds(
        ...
        Args:
            ...
            contact_template: Pre-packed SapContactData with static fields.
        ...
        loop_count = sap_trim_all_in_place(m_a, m_a_count, m_b, m_b_count)

        loop_count = sap_remove_zero_length_edges(m_b, loop_count, SAP_MULTICONTACT_EPS)

        if loop_count > 1:
            result = wp.vec4i()
            if loop_count > 4:
                result = sap_approx_max_quadrilateral_area_with_calipers(m_b, loop_count)
                loop_count = 4
            else:
            ...
            for i in range(loop_count):
                ...
                a = sap_body_projector_project(projector_a, p_local, normal_local)
                b = sap_body_projector_project(projector_b, p_local, normal_local)
    ...
    def sap_build_manifold(
        ...
        tangent_a, tangent_b = sap_orthonormal_basis(normal)
        ...
        a_buffer = wp.array(ptr=sap_get_ptr(b_buffer) + wp.uint64(5 * 8), shape=(5,), dtype=wp.vec2f)
        ...
        for e in range(5):
            ...
            cos_tilt = SAP_COS_TILT_ANGLE
            c_sin = c * SAP_SIN_TILT_ANGLE
            s_sin = s * SAP_SIN_TILT_ANGLE
            ...
            a_count, was_added_a = sap_add_avoid_duplicates_vec2(a_buffer, a_count, pt_a_2d, SAP_MULTICONTACT_EPS)
            if was_added_a:
                plane_tracker_a = sap_update_incremental_plane_tracker(plane_tracker_a, pt_a_3d, a_count - 1)
            ...
            b_count, was_added_b = sap_add_avoid_duplicates_vec2(b_buffer, b_count, pt_b_2d, SAP_MULTICONTACT_EPS)
            if was_added_b:
                plane_tracker_b = sap_update_incremental_plane_tracker(plane_tracker_b, pt_b_3d, b_count - 1)
        ...
        if a_count < 2 or b_count < 2:
            ...
            projector_a, projector_b = sap_create_body_projectors(plane_tracker_a, p_a, plane_tracker_b, p_b, normal)

            if sap_excess_normal_deviation(normal, projector_a.normal) or sap_excess_normal_deviation(
                normal, projector_b.normal
            ):
                count_out = 0
                normal_dot = 0.0
            else:
                num_manifold_points, normal_dot = sap_extract_4_point_contact_manifolds(
                    a_buffer,
                    a_count,
                    b_buffer,
                    b_count,
        ...
        if sap_should_include_deepest_contact(normal_dot) or count_out == 0:
...
__all__ = [
    ...
    "SapBodyProjector",
    "SapIncrementalPlaneTracker",
    "sap_add_avoid_duplicates_vec2",
    "sap_approx_max_quadrilateral_area_with_calipers",
    "sap_body_projector_project",
    "sap_compute_line_segment_projector_normal",
    "sap_create_body_projectors",
    "sap_create_build_manifold",
    "sap_excess_normal_deviation",
    "sap_get_ptr",
    "sap_insert_vec2",
    "sap_intersection_point",
    "sap_ray_plane_intersection",
    "sap_remove_zero_length_edges",
    "sap_should_include_deepest_contact",
    "sap_signed_area",
    "sap_trim_all_in_place",
    "sap_trim_in_place",
    "sap_update_incremental_plane_tracker",
]
