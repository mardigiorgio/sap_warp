"""Sweep-and-prune broad phase for the Newton-compatible collision front end.
...
def _sap_normalize_sort_mode(mode: str) -> SapSortMode:
...
def _sap_create_tile_sort_kernel(tile_size: int):
...
def _sap_range_kernel(
    ...
    limit = sap_binary_search_segment(
...
def _process_single_sap_pair(
    ...
    if num_filter_pairs > 0 and sap_is_pair_excluded(pair, filter_pairs, num_filter_pairs):
        return
...
def _sap_broadphase_kernel(
    ...
    while workid < total_work_packages:
        ...
        flat_id = sap_binary_search(sap_cumulative_sum_in, workid, 0, world_count * max_shapes_per_world)
        ...
        if sap_test_world_and_group_pair(world1, world2, col_group1, col_group2):
...
class SapBroadPhaseSAP:
    ...
    def __init__(
        ...
        self.sort_type = _sap_normalize_sort_mode(sort_type)
        ...
        if self.sort_type == "tile":
            ...
            self.tile_sort_kernel = _sap_create_tile_sort_kernel(self.tile_size)
...
__all__ = [
    ...
    "_sap_create_tile_sort_kernel",
    "_sap_normalize_sort_mode",
    "_sap_project_aabb",
    "_sap_project_kernel",
    "_sap_range_kernel",
