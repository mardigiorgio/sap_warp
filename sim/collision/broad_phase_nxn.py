"""NxN broad phase for the Newton-compatible SAP collision front end.
...
@wp.kernel
def _sap_nxn_broadphase_precomputed_pairs(
    # Input arrays
...
@wp.func
def _sap_get_lower_triangular_indices(index: int, matrix_size: int) -> tuple[int, int]:
...
@wp.func
def _sap_find_world_and_local_id(
    tid: int,
...
@wp.kernel
def _sap_nxn_broadphase_kernel(
    # Input arrays
    shape_bounding_box_lower: wp.array(dtype=wp.vec3, ndim=1),
    shape_bounding_box_upper: wp.array(dtype=wp.vec3, ndim=1),
    shape_gap: wp.array(dtype=float, ndim=1),  # Optional per-shape effective gaps (can be empty if AABBs pre-expanded)
    ...
    world_id, local_id = _sap_find_world_and_local_id(tid, world_cumsum_lower_tri)
    ...
    local_shape1, local_shape2 = _sap_get_lower_triangular_indices(local_id, num_shapes_in_world)
    ...
    if not sap_test_world_and_group_pair(world1, world2, collision_group1, collision_group2):
        return
    ...
    if sap_check_aabb_overlap(
        ...
        if num_filter_pairs > 0 and sap_is_pair_excluded(wp.vec2i(shape1, shape2), filter_pairs, num_filter_pairs):
            return
        sap_write_pair(
...
class SapBroadPhaseAllPairs:
    ...
    def __init__(
        ...
        index_map_np, slice_ends_np = sap_precompute_world_map(shape_world_np, shape_flags_np)

        # Calculate number of regular worlds (excluding dedicated -1 segment at end)
        # Must be derived from filtered slices since sap_precompute_world_map applies flags
...
class SapBroadPhaseExplicit:
    ...
    def launch(
        ...
        wp.launch(
            kernel=_sap_nxn_broadphase_precomputed_pairs,
...
__all__ = [
    ...
    "_sap_find_world_and_local_id",
    "_sap_get_lower_triangular_indices",
    "_sap_nxn_broadphase_kernel",
    "_sap_nxn_broadphase_precomputed_pairs",
]
