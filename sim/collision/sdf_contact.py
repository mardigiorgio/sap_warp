"""SDF contact generation for the Newton-compatible collision path.

Source note: the SAP modifications in this module are based on Newton's SDF
collision code and adapted for SAP Warp contact buffers.
"""
...
from .contact_data import SAP_SHAPE_PAIR_HFIELD_BIT, SAP_SHAPE_PAIR_INDEX_MASK, SapContactData
from .global_reduction import SapGlobalContactReducerData, sap_export_and_reduce_contact_centered
from .heightfield import SapHeightfieldData, sap_sample_sdf_grad_heightfield, sap_sample_sdf_heightfield
from .reduction import (
    sap_create_shared_memory_pointer_block_dim_mul_func,
    sap_get_shared_memory_pointer_block_dim_plus_2_ints,
    sap_synchronize,
)
from .sdf_texture import SapTextureSDFData, sap_texture_sample_sdf, sap_texture_sample_sdf_grad
...
_sap_get_shared_memory_vertex_cache = sap_create_shared_memory_pointer_block_dim_mul_func(9)


@wp.func
def sap_scale_sdf_result_to_world(
    distance: float,
    gradient: wp.vec3,
    sdf_scale: wp.vec3,
...
@wp.func
def sap_sample_sdf_grad_using_mesh(
    mesh_id: wp.uint64,
    world_pos: wp.vec3,
    max_dist: float = 1000.0,
) -> tuple[float, wp.vec3]:
...
@wp.func
def sap_closest_pt_point_bary_triangle(c: wp.vec3) -> wp.vec3:
    """
...
@wp.func
def sap_get_triangle_from_mesh(
    mesh_id: wp.uint64,
    mesh_scale: wp.vec3,
...
@wp.func
def sap_get_bounding_sphere(v0: wp.vec3, v1: wp.vec3, v2: wp.vec3) -> tuple[wp.vec3, float]:
    """
...
@wp.func
def sap_add_to_shared_buffer_atomic(
    thread_id: int,
    add_triangle: bool,
    tri_idx: int,
    ...
    sap_synchronize()  # SYNC 1: All atomic writes and progress update complete
    ...
    sap_synchronize()  # SYNC 2: All corrections complete, buffer consistent


@wp.func
def sap_get_triangle_from_heightfield(
    ...
    Returns ``(v0_world, v1_world, v2_world)`` matching :func:`sap_get_triangle_from_mesh`.
    """
...
@wp.func
def sap_get_triangle_count(shape_type: int, mesh_id: wp.uint64, hfd: SapHeightfieldData) -> int:
    """Return the number of triangles for a mesh or heightfield shape."""
    if shape_type == SapGeoType.HFIELD:
...
def _sap_create_sdf_contact_funcs(enable_heightfields: bool):
    ...
    @wp.func
    def sap_do_triangle_sdf_collision_func(
        texture_sdf: SapTextureSDFData,
        ...
        if wp.static(enable_heightfields):
            if sdf_is_heightfield:
                dist = sap_sample_sdf_heightfield(hfd_sdf, elevation_data, p)
                d0 = sap_sample_sdf_heightfield(hfd_sdf, elevation_data, v0)
                d1 = sap_sample_sdf_heightfield(hfd_sdf, elevation_data, v1)
                d2 = sap_sample_sdf_heightfield(hfd_sdf, elevation_data, v2)
            elif use_bvh_for_sdf:
                dist = sap_sample_sdf_using_mesh(sdf_mesh_id, p)
                d0 = sap_sample_sdf_using_mesh(sdf_mesh_id, v0)
                d1 = sap_sample_sdf_using_mesh(sdf_mesh_id, v1)
                d2 = sap_sample_sdf_using_mesh(sdf_mesh_id, v2)
            else:
                dist = sap_texture_sample_sdf(texture_sdf, p)
                d0 = sap_texture_sample_sdf(texture_sdf, v0)
                d1 = sap_texture_sample_sdf(texture_sdf, v1)
                d2 = sap_texture_sample_sdf(texture_sdf, v2)
        else:
            if use_bvh_for_sdf:
                dist = sap_sample_sdf_using_mesh(sdf_mesh_id, p)
                d0 = sap_sample_sdf_using_mesh(sdf_mesh_id, v0)
        ...
        for _iter in range(16):
            if wp.static(enable_heightfields):
                if sdf_is_heightfield:
                    _, sdf_gradient = sap_sample_sdf_grad_heightfield(hfd_sdf, elevation_data, p)
                elif use_bvh_for_sdf:
                    _, sdf_gradient = sap_sample_sdf_grad_using_mesh(sdf_mesh_id, p)
                else:
                    _, sdf_gradient = sap_texture_sample_sdf_grad(texture_sdf, p)
            else:
                if use_bvh_for_sdf:
                    _, sdf_gradient = sap_sample_sdf_grad_using_mesh(sdf_mesh_id, p)
                else:
            ...
            new_uvw = sap_closest_pt_point_bary_triangle(new_uvw)
        ...
        if wp.static(enable_heightfields):
            if sdf_is_heightfield:
                dist, sdf_gradient = sap_sample_sdf_grad_heightfield(hfd_sdf, elevation_data, p)
            elif use_bvh_for_sdf:
                dist, sdf_gradient = sap_sample_sdf_grad_using_mesh(sdf_mesh_id, p)
            else:
                dist, sdf_gradient = sap_texture_sample_sdf_grad(texture_sdf, p)
        else:
            if use_bvh_for_sdf:
                dist, sdf_gradient = sap_sample_sdf_grad_using_mesh(sdf_mesh_id, p)
            else:
    ...
    @wp.func
    def sap_find_interesting_triangles_func(
        thread_id: int,
        mesh_scale: wp.vec3,
        ...
        if wp.static(enable_heightfields):
            sdf_is_heightfield = sdf_shape_type == SapGeoType.HFIELD
        else:
        ...
        sap_synchronize()  # Ensure buffer state is consistent before starting
        ...
        while buffer[capacity + 1] < tri_end and buffer[capacity] < capacity:
            ...
            if tri_idx < tri_end:
                if wp.static(enable_heightfields):
                    if tri_shape_type == SapGeoType.HFIELD:
                        v0_scaled, v1_scaled, v2_scaled = sap_get_triangle_from_heightfield(
                            hfd_tri, elevation_data, mesh_scale, mesh_to_sdf_transform, tri_idx
                        )
                    else:
                        v0_scaled, v1_scaled, v2_scaled = sap_get_triangle_from_mesh(
                ...
                bounding_sphere_center, bounding_sphere_radius = sap_get_bounding_sphere(v0, v1, v2)

                threshold = bounding_sphere_radius + contact_distance

                if sdf_is_heightfield:
                    sdf_dist = sap_sample_sdf_heightfield(hfd_sdf, elevation_data, bounding_sphere_center)
                    add_triangle = sdf_dist <= threshold
                elif use_bvh_for_sdf:
                    sdf_dist = sap_sample_sdf_using_mesh(sdf_mesh_id, bounding_sphere_center, 1.01 * threshold)
                    add_triangle = sdf_dist <= threshold
                else:
                    ...
                    if aabb_dist_sq > culling_radius * culling_radius:
                        ...
                        sdf_dist = sap_texture_sample_sdf(texture_sdf, bounding_sphere_center)
                        add_triangle = sdf_dist <= culling_radius

            sap_synchronize()  # Ensure all threads have read base_tri_idx before any writes
            ...
            # sap_add_to_shared_buffer_atomic ends with sync, buffer is consistent for next while check

        sap_synchronize()  # Final sync before returning

    return sap_find_interesting_triangles_func, sap_do_triangle_sdf_collision_func


@wp.kernel(enable_backward=False)
def sap_compute_mesh_mesh_block_offsets(
    ...
    for i in range(pair_count):
        ...
        has_hfield = (pair_encoded[0] & SAP_SHAPE_PAIR_HFIELD_BIT) != 0
        pair = wp.vec2i(pair_encoded[0] & SAP_SHAPE_PAIR_INDEX_MASK, pair_encoded[1])
        for mode in range(2):
            is_hfield = has_hfield and mode == 0
            shape_idx = pair[mode]
            if is_hfield:
                hfd = heightfield_data[shape_heightfield_index[shape_idx]]
                total_tris += sap_get_triangle_count(SapGeoType.HFIELD, wp.uint64(0), hfd)
            else:
    ...
    for i in range(pair_count):
        ...
        for mode in range(2):
            ...
            if is_hfield:
                hfd = heightfield_data[shape_heightfield_index[shape_idx]]
                pair_tris += sap_get_triangle_count(SapGeoType.HFIELD, wp.uint64(0), hfd)
            else:
                mesh_id = shape_source[shape_idx]
                if mesh_id != wp.uint64(0):
...
def sap_create_narrow_phase_process_mesh_mesh_contacts_kernel(
    writer_func: Any,
    enable_heightfields: bool = True,
    reduce_contacts: bool = False,
):
    find_interesting_triangles, do_triangle_sdf_collision = _sap_create_sdf_contact_funcs(enable_heightfields)

    @wp.kernel(enable_backward=False, module="unique")
    def sap_mesh_sdf_collision_kernel(
        shape_data: wp.array(dtype=wp.vec4),
        shape_transform: wp.array(dtype=wp.transform),
        shape_source: wp.array(dtype=wp.uint64),
        texture_sdf_table: wp.array(dtype=SapTextureSDFData),
        ...
        for pair_idx in range(block_id, pair_count, total_num_blocks):
            ...
            for mode in range(2):
                ...
                if wp.static(enable_heightfields):
                    ...
                    tri_is_hfield = False
                    sdf_is_hfield = False
                tri_type = SapGeoType.HFIELD if tri_is_hfield else SapGeoType.MESH
                sdf_type = SapGeoType.HFIELD if sdf_is_hfield else SapGeoType.MESH

                mesh_id_tri = shape_source[tri_shape]
                mesh_id_sdf = shape_source[sdf_shape]

                # Skip invalid sources (heightfields use SapHeightfieldData instead of mesh id)
                ...
                selected_triangles = wp.array(
                    ptr=sap_get_shared_memory_pointer_block_dim_plus_2_ints(),
                    shape=(wp.block_dim() + 2,),
                    dtype=wp.int32,
                )
                vertex_cache = wp.array(
                    ptr=_sap_get_shared_memory_vertex_cache(),
                ...
                num_tris = sap_get_triangle_count(tri_type, mesh_id_tri, hfd_tri)
                ...
                while selected_triangles[tri_capacity + 1] < num_tris:
                    ...
                    if has_triangle:
                        ...
                        dist, direction = sap_scale_sdf_result_to_world(
    ...
    if not reduce_contacts:
        return sap_mesh_sdf_collision_kernel
    ...
    @wp.kernel(enable_backward=False, module="unique")
    def sap_mesh_sdf_collision_global_reduce_kernel(
        shape_data: wp.array(dtype=wp.vec4),
        shape_transform: wp.array(dtype=wp.transform),
        shape_source: wp.array(dtype=wp.uint64),
        texture_sdf_table: wp.array(dtype=SapTextureSDFData),
    ...
    return sap_mesh_sdf_collision_global_reduce_kernel
...
__all__ = [
    ...
    "sap_get_triangle_from_heightfield",
    "sap_get_triangle_from_mesh",
