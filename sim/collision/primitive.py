...
primitive collision code and adapted for SAP Warp's narrow-phase stages.
...
Returns (multi contact): (distances: vecN, positions: matNx3, normals: vecN or matNx3)
                        Use SAP_MAXVAL for unpopulated contact slots.
"""
...
from .math import SAP_MAXVAL, sap_normalize_with_norm, sap_safe_div

# Local type definitions for use within kernels
_sap_vec8f = wp.types.vector(8, wp.float32)
_sap_mat23f = wp.types.matrix((2, 3), wp.float32)
_sap_mat43f = wp.types.matrix((4, 3), wp.float32)
_sap_mat83f = wp.types.matrix((8, 3), wp.float32)
...
SAP_CYLINDER_FLAT_MODE_DEG = 22.5
SAP_CYLINDER_FLAT_MODE_COS = float(math.cos(math.radians(SAP_CYLINDER_FLAT_MODE_DEG)))


@wp.func
def sap_closest_segment_point(a: wp.vec3, b: wp.vec3, pt: wp.vec3) -> wp.vec3:
...
@wp.func
def sap_closest_segment_point_and_dist(a: wp.vec3, b: wp.vec3, pt: wp.vec3) -> tuple[wp.vec3, float]:
    """Returns closest point on the line segment and the distance squared."""
    closest = sap_closest_segment_point(a, b, pt)
    dist = wp.dot((pt - closest), (pt - closest))
    return closest, dist


@wp.func
def sap_closest_segment_to_segment_points(a0: wp.vec3, a1: wp.vec3, b0: wp.vec3, b1: wp.vec3) -> tuple[wp.vec3, wp.vec3]:
    """Returns closest points between two line segments."""

    dir_a, len_a = sap_normalize_with_norm(a1 - a0)
    dir_b, len_b = sap_normalize_with_norm(b1 - b0)
    ...
    new_a, d1 = sap_closest_segment_point_and_dist(a0, a1, best_b)
    new_b, d2 = sap_closest_segment_point_and_dist(b0, b1, best_a)
    if d1 < d2:
        return new_a, best_b
...
@wp.func
def sap_collide_sphere_capsule(
    # In:
    sphere_pos: wp.vec3,
    sphere_radius: float,
    capsule_pos: wp.vec3,
    ...
    pt = sap_closest_segment_point(capsule_pos - segment, capsule_pos + segment, sphere_pos)

    # Use sphere-sphere collision between sphere and closest point
    return sap_collide_sphere_sphere(sphere_pos, sphere_radius, pt, capsule_radius)


@wp.func
def sap_collide_capsule_capsule(
    # In:
    cap1_pos: wp.vec3,
    cap1_axis: wp.vec3,
    cap1_radius: float,
    ...
    Returns:
      Tuple containing:
        contact_dist: Vector of contact distances (SAP_MAXVAL for invalid contacts)
    ...
    """
    contact_dist = wp.vec2(SAP_MAXVAL, SAP_MAXVAL)
    contact_pos = _sap_mat23f()
    ...
    if wp.abs(det) >= SAP_MINVAL:
        ...
        dist, pos, normal = sap_collide_sphere_sphere(vec1, cap1_radius, vec2, cap2_radius)
        contact_dist[0] = dist
        contact_pos[0] = pos
    ...
    # Parallel axes: 2 contacts (use first contact's normal for both)
        ...
        x2 = wp.clamp((v + mb) / mc, -1.0, 1.0)
        vec2 = cap2_pos + axis2 * x2
        dist, pos, _normal = sap_collide_sphere_sphere(vec1, cap1_radius, vec2, cap2_radius)
        contact_dist[1] = dist
        contact_pos[1] = pos
...
def sap_collide_plane_capsule(
    ...
    b, b_norm = sap_normalize_with_norm(axis - n * wp.dot(n, axis))
    ...
    dist1, pos1 = sap_collide_plane_sphere(n, plane_pos, capsule_pos + segment, capsule_radius)

    # Second contact (negative end of capsule)
    dist2, pos2 = sap_collide_plane_sphere(n, plane_pos, capsule_pos - segment, capsule_radius)

    dist = wp.vec2(dist1, dist2)
    pos = _sap_mat23f(pos1[0], pos1[1], pos1[2], pos2[0], pos2[1], pos2[2])

    return dist, pos, frame


@wp.func
def sap_collide_plane_ellipsoid(
    # In:
    plane_normal: wp.vec3,
    plane_pos: wp.vec3,
...
def sap_collide_plane_box(
    ...
    Returns:
      Tuple containing:
        contact_dist: Vector of contact distances (SAP_MAXVAL for unpopulated contacts)
        contact_pos: Matrix of contact positions (one per row)
...
@wp.func
def sap_collide_sphere_cylinder(
    # In:
    sphere_pos: wp.vec3,
    sphere_radius: float,
    ...
    if collide_side:
        ...
        return sap_collide_sphere_sphere(sphere_pos, sphere_radius, pos_target, cylinder_radius)
    # cap collision
    elif collide_cap:
        if x > 0.0:
            # top cap
        ...
        dist, pos = sap_collide_plane_sphere(plane_normal, pos_cap, sphere_pos, sphere_radius)
        return dist, pos, -plane_normal  # flip normal after position calculation
    # corner collision
    else:
        inv_len = sap_safe_div(1.0, wp.sqrt(p_proj_sqr))
        ...
        return sap_collide_sphere_sphere(sphere_pos, sphere_radius, pos_corner, 0.0)


@wp.func
def sap_collide_plane_cylinder(
    # In:
    plane_normal: wp.vec3,
    plane_pos: wp.vec3,
    ...
    Returns:
      Tuple containing:
        contact_dist: Vector of contact distances (SAP_MAXVAL for unpopulated).
        contact_pos: Matrix of contact positions (one per row).
        contact_normal: Contact normal (plane normal).
    """
    contact_dist = wp.vec4(SAP_MAXVAL)
    contact_pos = _sap_mat43f()
    ...
    flat_mode_cos = wp.static(SAP_CYLINDER_FLAT_MODE_COS)
...
@wp.func
def _sap_compute_rotmore(face_idx: int) -> wp.mat33:
...
def sap_collide_box_box(
    ...
    contact_dist = _sap_vec8f()
    for i in range(8):
        contact_dist[i] = SAP_MAXVAL
    contact_pos = _sap_mat83f()
    contact_normals = _sap_mat83f()
    contact_count = 0
    ...
    if axis_code < 12:
        ...
        box_idx = axis_code // 6
        rotmore = _sap_compute_rotmore(face_idx)
        ...
        if dirs == 2:
            ...
            ay = cn1[1]
            by = cn2[1]
            C = sap_safe_div(1.0, ax * by - bx * ay)
        ...
        rotmore = _sap_compute_rotmore(wp.where(cle1 & (1 << pax2), pax2, pax2 + 3))
...
def sap_collide_sphere_box(
    ...
    clamped_dir, dist = sap_normalize_with_norm(clamped - center)
...
def sap_collide_capsule_box(
    ...
    if cltype == -4:  # invalid type
        return wp.vec2(SAP_MAXVAL), _sap_mat23f(), _sap_mat23f()
    ...
    if cltype >= 0 and cltype // 3 != 1:  # closest to a corner of the box
        ...
        if c1 != 0 and c1 != 7:  # create second contact point
            ...
            if axis[ax] * axis[ax] > 0.5:  # second point along the edge of the box
                m = 2.0 * sap_safe_div(box_size[ax], wp.abs(halfaxis[ax]))
                secondpos = wp.min(1.0 - wp.float32(mul) * bestsegmentpos, m)
            else:  # second point along a face of the box
                # check for overshoot again
                m = 2.0 * wp.min(
                    sap_safe_div(box_size[ax1], wp.abs(halfaxis[ax1])),
                    sap_safe_div(box_size[ax2], wp.abs(halfaxis[ax2])),
                )
                secondpos = -wp.min(1.0 + wp.float32(mul) * bestsegmentpos, m)
        ...
        if c1 == 1 or c1 == 2 or c1 == 4:  # create second contact point
            ...
            e1 = 2.0 * sap_safe_div(box_size[ax2], wp.abs(halfaxis[ax2]))
            ...
            e1 = box_size[ax] * sap_safe_div(e2, wp.abs(halfaxis[ax]))
        ...
        if clface != -1:  # create second contact point
            ...
            for i in range(3):
                if i != clface:
                    ha_r = sap_safe_div(wp.float32(mul), halfaxis[i])
    ...
    dist1, pos1, normal1 = sap_collide_sphere_box(s1_pos_g, capsule_radius, box_pos, box_rot, box_size)
    ...
    if secondpos > -3:  # secondpos was modified
        ...
        # collide with sphere using core function
        dist2, pos2, normal2 = sap_collide_sphere_box(s2_pos_g, capsule_radius, box_pos, box_rot, box_size)
    else:
        dist2 = SAP_MAXVAL
        pos2 = wp.vec3()
        normal2 = wp.vec3()
    ...
    return (
        wp.vec2(dist1, dist2),
        _sap_mat23f(pos1[0], pos1[1], pos1[2], pos2[0], pos2[1], pos2[2]),
        _sap_mat23f(normal1[0], normal1[1], normal1[2], normal2[0], normal2[1], normal2[2]),
    )


__all__ = [
    "SAP_CYLINDER_FLAT_MODE_COS",
    "SAP_CYLINDER_FLAT_MODE_DEG",
    "SAP_MINVAL",
    "_sap_mat23f",
    "_sap_mat43f",
    "_sap_mat83f",
    "_sap_vec8f",
    "sap_closest_segment_point",
    "sap_closest_segment_point_and_dist",
    "sap_closest_segment_to_segment_points",
    ...
    "_sap_compute_rotmore",
    "sap_collide_box_box",
    "sap_collide_sphere_box",
    "sap_collide_capsule_box",
]
